...
typedef struct {
    uintptr_t* ring;        // Fixed-size ring buffer of addresses
    khash_t(ghost)* hash;   // addr -> ring index for O(1) lookup
    uint32_t head;          // Next write position (overwrites oldest)
...
static struct {
    ...
    s3fifo_queue_t in_used_queue;  // Overflow queue for blocked blocks
    s3fifo_ghost_t ghost;
    uint32_t initial_capacity;     // For max capacity calculation
    int initialized;
} s3fifo = {0};
...
static void s3fifo_init(uint32_t capacity) {
    ...
    s3fifo.small_queue.capacity = capacity / 10;
    ...
    s3fifo.in_used_queue.capacity = s3fifo.small_queue.capacity;
    s3fifo.initial_capacity = s3fifo.small_queue.capacity + s3fifo.main_queue.capacity;

    uint32_t ghost_cap = next_power_of_2(capacity);
    s3fifo.ghost.capacity = ghost_cap;
...
// Recheck in_used_queue and move idle blocks back to small queue
// Always does full scan - called when eviction is needed
static void s3fifo_recheck_in_used_queue(void) {
    ...
    while (db) {
        dynablock_t* next = db->s3fifo_next;
...
// Maximum capacity growth: 4x initial capacity
#define S3FIFO_MAX_CAPACITY_MULTIPLIER 4

// S3-FIFO eviction - evict blocks until queue capacity is satisfied
// If all blocks are in_used and can't evict, increase capacity dynamically
int PurgeDynarecMap(void) {
    if (!s3fifo.initialized) return 0;

    uint32_t total_capacity = s3fifo.small_queue.capacity + s3fifo.main_queue.capacity;
    uint32_t max_capacity = s3fifo.initial_capacity * S3FIFO_MAX_CAPACITY_MULTIPLIER;
    ...
    while (total_count >= total_capacity) {
        ...
        if (s3fifo.main_queue.count > s3fifo.main_queue.capacity ||
            ...
            freed = s3fifo_try_evict_from_main();
        } else {
            freed = s3fifo_try_evict_from_small();
        }

        if (freed == 0) {
            // Can't evict - all evictable blocks are in_used
            // Increase capacity if within limit
            if (total_capacity < max_capacity) {
                uint32_t increase = s3fifo.initial_capacity / 10;
        ...
        evicted++;
        total_count = s3fifo.small_queue.count + s3fifo.main_queue.count + s3fifo.in_used_queue.count;
    }
