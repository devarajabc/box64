...
#define BLOCK_PROFILE_PROLOG()  do {                                    \
        /* Block profiling prolog: 6 instructions */                   \
        EMIT(0);  /* STP x4, x5, [SP, #-16]! */                        \
        TABLE64(x4, (uintptr_t)dyn->dynablock); /* LDR (in TABLE64) */ \
        EMIT(0);  /* ADD x4, x4, offset */                             \
        EMIT(0);  /* MOVZ x5, 1 */                                     \
        EMIT(0);  /* LDADD x5, xZR, [x4] */                            \
        EMIT(0);  /* LDP x4, x5, [SP], #16 */                          \
    } while(0)
