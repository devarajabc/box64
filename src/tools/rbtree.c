...
{
    return (a < b) ? _CMP_LESS : (a > b) ? _CMP_GREATER
                                         : _CMP_EQUAL;
}
...
{
    node->right_red = (rbnode*)(((uintptr_t)right) | (((uintptr_t)node->right_red) & 1));
}
...
int main()
{
    ...
    rb_set(tree, 0x210000, 0x270000, 100);
    // check
    ...
    rb_set(tree, 0x130000, 0x250000, 135);
    printf("case 2-1\n");
    ...
    val = rb_get(tree, 0x181994);
    printf("0x181994 has attribute %hhu\n", val);
    val = rb_get(tree, 0x211994);
    printf("0x181994 has attribute %hhu\n", val);
    val = rb_get(tree, 0x261994);
    printf("0x181994 has attribute %hhu\n", val);
    // rbtree_print(tree);
    fflush(stdout);
